Ext.define("MainHub.view.requests.FilepathsWindowController", {
  ...
  onSaveButtonClick: function () {
    ...
    if (this.isAdding) {
      ...
      for (var i = 0; i < userpathsArray.length; i++) {
      ...
      var currentEditId = this.currentEditId;
      for (var i = 0; i < userpathsArray.length; i++) {
    ...
    this.userpathsArray = Array.from(newInputData).map(function(entry, index) {
      return {
        pathId: index + 1,
        pathName: entry[0],
        pathValue: entry[1]
  ...
  generateModifiedUserpaths: function (userpathsArray, container) {
    ...
    if (userpathsArray.filter(function(item) { return item.pathValue !== null}).length === 0) {
      container.add({
        xtype: "label",
        text: "No User Paths",
        style: {
      ...
      userpathsArray.forEach(function (obj) {
  ...
  editUserpath: function (pathId) {
    ...
    var userpath = this.userpathsArray.find(function (obj){ return obj.pathId === pathId});
  ...
  deleteUserpath: function (pathId) {
    ...
    var userpathIndex = userpathsArray.findIndex(
      function (obj) { return obj.pathId === pathId }
    );
