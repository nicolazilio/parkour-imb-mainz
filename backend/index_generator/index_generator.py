...
class IndexRegistry:
    ...
    def fetch_pairs(self, index_type, char_coord, num_coord, direction):
        ...
        for pair in index_pairs:
            ...
            self.pairs[index_type.pk].append(Pair(index1, index2, pair.coordinate))
    ...
    def get_diagonal(self, index_pairs):
        ...
        # H1, H2, ... , H12
        coord_matrix = [
            [char + str(num + 1) for num in range(num_coord)] for char in list(rows)
        ]
    ...
    def get_indices(self, index_type_id, index_group):
        ...
        """
        return self.indices.get(index_type_id, {"i7": [], "i5": []}).get(
            index_group, []
        )
    ...
    def to_list(self, format, index_type, read_type, indices):
        ...
        return list(
            map(
                lambda x: self.create_index_dict(
                    format, index_type, read_type, x.prefix, x.number, x.index
                ),
                indices,
            )
        )
...
class IndexGenerator:
    ...
    def __init__(self, library_ids, sample_ids, start_coord, direction, sequencer_chemistry, min_hamming_distance):
        ...
        self.libraries = (
            Library.objects.filter(pk__in=library_ids)
            .select_related(
                "read_length",
                "index_type",
            )
    ...
    def validate_index_types(self, records):
        ...
        if len(index_types) != len(records):
            raise ValueError("Index Type must be set for all libraries and samples.")
    ...
    def generate(self):
        ...
        if not any(self._result) and self.num_samples == 1:
            ...
            self._result.append(
                self.create_result_dict(self.samples[0], index_i7, index_i5)
            )
            return self.result
        ...
        if (
            len(plate_samples) > self.MAX_RANDOM_SAMPLES
            or self.samples[0].index_type.read_type == "long"
        ):
        ...
        while not is_ok and attempt < self.MAX_ATTEMPTS:
            ...
            indices_i7 = self.find_indices(tube_samples, depths, "i7", init_i7)
            if self.mode == "single":
                indices_i5 = [self.index_registry.create_index_dict()] * len(indices_i7)
            else:
    ...
    def find_random(self, sample):
        ...
        if sample.index_type.format == "single":
            ...
            pair = random.choice(self.index_registry.get_pairs(sample.index_type.pk))
    ...
    def find_indices(self, samples, depths, index_group, init_indices):
        ...
        for sample in samples:
            index = self.find_index(sample, index_group, indices, depths)
        ...
        if len(indices) == len(init_indices) + len(samples):
            ...
            plate_indices = [
                x for x in indices if x["format"] == "plate" and not x["is_library"]
            ]
            tube_indices = [
                x for x in indices if x["format"] == "single" and not x["is_library"]
            ]
            return library_indices + plate_indices + self.sort_indices(tube_indices)
        raise ValueError(
            f'Could not generate indices "{index_group}" ' + "for the selected samples."
        )
    ...
    def find_index(self, sample, index_group, current_indices, depths):
        ...
        indices = list(
            self.index_registry.get_indices(sample.index_type.pk, index_group)
        )
    ...
    def find_pairs(self, samples, depths, init_pairs):
        ...
        raise ValueError(
            "Could not generate index pairs for the " + "selected samples."
        )
    ...
    def find_pair(self, sample, depths, current_pairs):
        ...
        for pair in pairs:
            if sample.index_type.read_type == "long":
                ...
                converted_index = self.convert_index(self._concat_index_pair(pair))
                scores = self.calculate_scores(
                    sample, converted_index, color_distribution, total_depth
                )
    ...
    def find_pairs_fixed(self, plate_samples, init_index_pairs):
        ...
        for index_type_id, samples in samples_dict.items():
            ...
            if len(samples) > len(pairs):
                raise IndexError(
                    f"Not enough indices of type {sample.index_type} for given number of samples"
                )
    ...
    def calculate_scores(
        ...
        for cycle in range(len(current_converted_index)):
            ...
            if distribution[cycle]["G"] > 0 and distribution[cycle]["R"] > 0:
                difference = (
                    abs(
                        (distribution[cycle]["G"] - distribution[cycle]["R"])
                        / total_depth
                    )
                    * 100
                )
    ...
    def _concat_index_pair(self, pair):
        return (
            pair.index1["index"] + pair.index2["index"]
            if self.mode == "dual"
