...
def create_index_type(indices_i7, indices_i5=None, format="single"):
    ...
    for idx in indices_i7:
        index = IndexI7(prefix=idx.prefix, number=idx.number, index=idx.index)
        index.save()
        index_type.indices_i7.add(index)

    if indices_i5:
        for idx in indices_i5:
            index = IndexI5(prefix=idx.prefix, number=idx.number, index=idx.index)
            index.save()
...
class TestIndexRegistry(BaseTestCase):
    ...
    def test_format_plate_mode_dual(self):
        ...
        self.assertEqual(
            coordinates, ["A1", "A2", "A3", "A4", "A5", "B1", "B2", "B3", "B4", "B5"]
        )
    ...
    def test_format_plate_direction_diagonal(self):
        ...
        self.assertEqual(coordinates, ["B4", "C5", "A4", "B5", "A5", "E1", "D1", "E2"])
...
class TestIndexGenerator(BaseTestCase):
    ...
    def test_save_pool(self):
        ...
        self.assertEqual(
            Pool.objects.filter(
                libraries__id__in=[library.pk], samples__id__in=[sample1.pk, sample2.pk]
            )
            .distinct()
            .count(),
            1,
    ...
    def test_one_sample_format_tube_mode_single(self):
        ...
        sample = create_sample(get_random_name(), index_type=self.index_type1)
    ...
    def test_one_sample_format_tube_mode_dual(self):
        sample = create_sample(get_random_name(), index_type=self.index_type2)
    ...
    def test_one_sample_format_plate_mode_single(self):
        ...
        self.assertEqual(
            IndexPair.objects.filter(index1=index_i7, index2=None).count(), 1
        )
    ...
    def test_one_sample_format_plate_mode_dual(self):
        ...
        self.assertEqual(
            IndexPair.objects.filter(index1=index_i7, index2=index_i5).count(), 1
        )
    ...
    def test_samples_formats_plate_and_tube_mode_dual(self):
        index_i7_ids_1 = [x.index_id for x in self.index_type5.indices_i7.all()]
        index_i5_ids_1 = [x.index_id for x in self.index_type5.indices_i5.all()]
        index_i7_ids_2 = [x.index_id for x in self.index_type2.indices_i7.all()]
        index_i5_ids_2 = [x.index_id for x in self.index_type2.indices_i5.all()]
    ...
    def test_save_pool_not_unique(self):
        ...
        self.assertEqual(data["message"], "Some of the indices are not unique.")
    ...
    def test_save_pool_no_records(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertFalse(data["success"])
        self.assertEqual(data["message"], "No libraries nor samples have been provided")
    ...
    def test_index_type_not_set(self):
        ...
        sample2 = create_sample(get_random_name(), read_length=self.read_length)
    ...
    def test_mixed_single_dual_indices(self):
        ...
        data = response.json()
        self.assertFalse(data["success"])
        self.assertEqual(data["message"], "Mixed single/dual indices are not allowed.")
